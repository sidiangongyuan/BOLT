...
from opencood.data_utils.post_processor.base_postprocessor_multi_class \
    import BasePostprocessorMultiClass
from opencood.utils import box_utils_mc
from opencood.utils.box_overlaps import bbox_overlaps
from opencood.visualization import vis_utils_mc


class VoxelPostprocessor3Heads(BasePostprocessorMultiClass):
    def __init__(self, anchor_params, train):
        super(VoxelPostprocessor3Heads, self).__init__(anchor_params, train)
    ...
    def generate_label(self, **kwargs):
        ...
        for i, (anchor_class_name, anchors, anchor_num) in enumerate(zip(self.anchor_class_names, anchors_list, num_anchors_per_location)):
            ...
            # (n, 8, 3)
            gt_box_corner_valid = \
                box_utils_mc.boxes_to_corners_3d(gt_box_center_valid[:, :7],
                                              self.params['order'])
            # (H*W*anchor_num, 8, 3)
            anchors_corner = \
                box_utils_mc.boxes_to_corners_3d(anchors,
                                              order=self.params['order'])
            # (H*W*anchor_num, 4)
            anchors_standup_2d = \
                box_utils_mc.corner2d_to_standup_box(anchors_corner)
            # (n, 4)
            gt_standup_2d = \
                box_utils_mc.corner2d_to_standup_box(gt_box_corner_valid)
    ...
    def post_process(self, data_dict, output_dict, projection=True):
        ...
        for cav_id, cav_content in data_dict.items():
            ...
            prob = output_dict[cav_id]['cls_preds'] # psm
            ...
            # regression map
            reg = output_dict[cav_id]['reg_preds'] # rm
            ...
            if len(boxes3d) != 0:
                # (N, 8, 3)
                boxes3d_corner = \
                    box_utils_mc.boxes_to_corners_3d(boxes3d,
                                                  order=self.params['order'])
                unprojected_box3d_list.append(boxes3d_corner.clone())

                # (N, 8, 3)
                projected_boxes3d = \
                    box_utils_mc.project_box3d(boxes3d_corner,
                                            transformation_matrix)

                # convert 3d bbx to 2d, (N,4)
                projected_boxes2d = \
                    box_utils_mc.corner_to_standup_box_torch(projected_boxes3d)
                # (N, 5)
                boxes2d_score = \
        ...
        # remove large bbx
        keep_index_1 = box_utils_mc.remove_large_pred_bbx(pred_box3d_tensor)
        keep_index_2 = box_utils_mc.remove_bbx_abnormal_z(pred_box3d_tensor)
        keep_index = torch.logical_and(keep_index_1, keep_index_2)
        # print(keep_index_1.sum().cpu())
        # print(keep_index_2.sum().cpu())
        # print(keep_index.sum().cpu())
        # print(pred_box3d_tensor.shape[0])
        ...
        # nms
        keep_index = box_utils_mc.nms_rotated(pred_box3d_tensor,
                                           scores,
        ...
        mask = \
            box_utils_mc.get_mask_for_boxes_within_range_torch(pred_box3d_tensor)
    ...
    def visualize(pred_box_tensor, gt_box_tensor, origin_lidar, map_lidar, show_vis, save_path,
        ...
        """
        vis_utils_mc.visualize_single_sample_output_gt(pred_box_tensor,
                                                    gt_box_tensor,
                                                    origin_lidar,
                                                    map_lidar,
                                                    show_vis,
