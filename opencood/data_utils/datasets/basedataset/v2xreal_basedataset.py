# -*- coding: utf-8 -*-
# Author: Huizhi Zhang <hzhang952@wisc.edu>
...
class V2XREALBaseDataset(Dataset):
    ...
    def __init__(self, params, visualize, train=True, calibrate=False):
        ...
        self.load_depth_file = False # v2xreal does not have depth data
        ...
        # hardcoded to exclude 2023-04-07 from the dataset since it only contains two cavs and no infra
    ...
    def retrieve_base_data(self, idx, cur_ego_pose_flag=True):
        ...
        for cav_id, cav_content in scenario_database.items():
            ...
            if timestamp_index - timestamp_delay <= 0:
                timestamp_delay = timestamp_index # no delay, preserve the original timestamp
            timestamp_index_delay = max(0, timestamp_index - timestamp_delay)
            timestamp_key_delay = self.return_timestamp_key(scenario_database,
                                                            timestamp_index_delay)
            data[cav_id]['time_delay'] = timestamp_delay
            ...
            # self.map_class_name_to_super_class_name(data[cav_id]['params']['vehicles'])
            # data[cav_id]['params']['vehicles'] = self.filter_boxes_by_class(data[cav_id]['params']['vehicles'])

            if self.load_camera_file:
                data[cav_id]['camera_data'] = \
                    load_camera_data(cav_content[timestamp_key_delay]['cameras'])

            # load lidar file
            if self.load_lidar_file or self.visualize:
                data[cav_id]['lidar_np'] = \
                    pcd_utils.load_lidar_bin(cav_content[timestamp_key_delay]['lidar'],
                                            zero_intensity=True)

            if getattr(self, "heterogeneous", False):
                data[cav_id]['modality_name'] = cav_content[timestamp_key_delay]['modality_name']
    ...
    def reform_param(self, cav_content, ego_content, timestamp_cur,
        ...
        delay_params['vehicles'] = self.filter_boxes_by_class(cur_params['vehicles'])
    ...
    def load_camera_files(cav_path, timestamp):
        ...
        camera1_file = os.path.join(cav_path,
                                    timestamp + '_cam1.jpeg')
        camera2_file = os.path.join(cav_path,
                                    timestamp + '_cam2.jpeg')
        camera3_file = os.path.join(cav_path,
                                    timestamp + '_cam3.jpeg') if os.path.exists(os.path.join(cav_path, timestamp + '_cam3.jpeg')) else None
        camera4_file = os.path.join(cav_path,
                                    timestamp + '_cam4.jpeg') if os.path.exists(os.path.join(cav_path, timestamp + '_cam4.jpeg')) else None
        camera_list = [camera for camera in [camera1_file, camera2_file, camera3_file, camera4_file] if camera is not None]
        return camera_list
