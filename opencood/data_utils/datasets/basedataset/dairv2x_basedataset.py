...
class DAIRV2XBaseDataset(Dataset):
    def __init__(self, params, visualize, train=True, calibrate=False):
        ...
        token_cfg = params.get("token", None)
        self.token_cfg = token_cfg if isinstance(token_cfg, dict) else None
        self.load_token_data = bool(self.token_cfg and self.token_cfg.get("enabled", False))
        self.token_cache_dir = self.token_cfg.get("cache_dir") if self.load_token_data else None
        ...
        heter_cfg = self.params.get("heter", None)
        if isinstance(heter_cfg, dict):
            self.heter_fix_ego = bool(heter_cfg.get("fix_ego", False))
        else:
            self.heter_fix_ego = False
    ...
    def retrieve_base_data(self, idx):
        ...
        if self.load_token_data:
            if not self.token_cache_dir:
                raise ValueError("token.enabled is true but token.cache_dir is not set")
            token_path = os.path.join(self.token_cache_dir, f"{veh_frame_id}.npz")
            if not os.path.exists(token_path):
                raise FileNotFoundError(f"Token cache not found: {token_path}")
            with np.load(token_path) as token_npz:
                data[0]["token_data"] = {
                    "xyz": token_npz["veh_token_xyz"],
                    "feat": token_npz["veh_token_feat"],
                    "mask": token_npz["veh_token_mask"],
                }
                data[1]["token_data"] = {
                    "xyz": token_npz["inf_token_xyz"],
                    "feat": token_npz["inf_token_feat"],
                    "mask": token_npz["inf_token_mask"],
                }
        ...
        if getattr(self, "heterogeneous", False):
            ...
            if self.train:  # optionally fix ego to vehicle side
                if not getattr(self, "heter_fix_ego", False):
                    p = np.random.rand()
                    if p > 0.5:
