...
def getIntermediateheter3classFusionDataset(cls):
    """
    cls: the Basedataset.
    """
    class Intermediateheter3ClassFusionDataset(cls):
        def __init__(self, params, visualize, train=True, calibrate=False):
            ...
            self.all_anchors, self.num_anchors_per_location = self.post_processor.generate_anchor_box()
        ...
        def get_item_single_car(self, selected_cav_base, ego_cav_base):
            ...
            if sensor_type == "camera":
                ...
                for idx, img in enumerate(camera_data_list):
                    idx += 1 if "v2xreal" in self.root_dir.lower() else 0
                    # skip cameras whose calibration is missing from the YAML
                    cam_key = "cam%d" % idx
            ...
            # anchor box
            selected_cav_processed.update({"anchor_box": self.all_anchors})
        ...
        def _getitem_impl(self, idx, retry=0):
            if retry >= 3:
                raise RuntimeError(f"Failed to load sample after 3 retries (last idx={idx})")
            try:
                return self._getitem_inner(idx)
            except (KeyError, RuntimeError) as e:
                import random
                new_idx = random.randint(0, len(self) - 1)
                print(f"[WARN] Skipping bad sample idx={idx} ({e}), retrying with idx={new_idx}")
                return self._getitem_impl(new_idx, retry + 1)
        ...
        def _getitem_inner(self, idx):
            ...
            for cav_id, selected_cav_base in base_data_dict.items():
                ...
                if distance > self.params['comm_range'] and self.calibrate == False:
            ...
            for _i, cav_id in enumerate(cav_id_list):
                ...
                if "v2xreal" not in self.root_dir.lower():
            ...
            object_bbx_center = \
                np.zeros((self.params['postprocess']['max_num'], 8)) # 8 instead of 7
