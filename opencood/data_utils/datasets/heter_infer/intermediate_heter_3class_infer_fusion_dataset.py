...
3 Class functionality added by Aiden Wong <aidenwong@ucla.edu>
...
def getIntermediateheter3classinferFusionDataset(cls):
    """
    cls: the Basedataset.
    """
    Intermediate_heter_3class_Fusion_Dataset = getIntermediateheter3classFusionDataset(cls)

    class Intermediate_heter_3class_Infer_Fusion_Dataset(Intermediate_heter_3class_Fusion_Dataset):
        """
        We add 'use_cav' to control the number of cars that really participate in collaboration.
        We use 'max_cav' to genenrate all gt boxes in the scene.
        """
        def __init__(self, params, visualize, train=True, calibrate=False):
            ...
            heter_cfg = params.get("heter", {})
            heter_cfg = heter_cfg if isinstance(heter_cfg, dict) else {}
            modality_setting = heter_cfg.get("modality_setting", {})
            modality_setting = modality_setting if isinstance(modality_setting, dict) else {}

            sensor_type_dict = {}
            for modality_name, setting in modality_setting.items():
                if not isinstance(setting, dict):
                    continue
                sensor_type = setting.get("sensor_type", None)
                if sensor_type is None:
                    continue
                sensor_type_dict[str(modality_name)] = str(sensor_type).lower()

            if not sensor_type_dict:
                sensor_type_dict = {
                    "m1": "lidar",
                    "m2": "lidar",
                    "m3": "lidar",
            ...
            self.sensor_type_dict = sensor_type_dict
            print("\n\nCaution: make sure your sensor type is consistent with:")
            print(self.sensor_type_dict)
            print("Otherwise, modify it in opencood/data_utils/datasets/heter_infer/intermediate_heter_3class_infer_fusion_dataset.py\n\n")
        ...
        def __getitem__(self, idx):
            ...
            for _i, cav_id in enumerate(cav_id_list):
                ...
                # 3-class postprocessor does not support camera object center;
                # always use lidar label in inference.
    ...
    return Intermediate_heter_3class_Infer_Fusion_Dataset
