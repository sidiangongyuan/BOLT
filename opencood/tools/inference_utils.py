...
def inference_late_fusion(batch_data, model, dataset):
    ...
    pred_box_tensor, pred_score, gt_box_tensor = result[0], result[1], result[2]

    return_dict = {"pred_box_tensor" : pred_box_tensor,
                    "pred_score" : pred_score,
                    "gt_box_tensor" : gt_box_tensor}
    if len(result) > 3:
        return_dict["gt_label_tensor"] = result[3]
    return return_dict
...
def inference_early_fusion(batch_data, model, dataset):
    ...
    if isinstance(res, (list, tuple)):
        pred_box_tensor = res[0]
        pred_score = res[1]
        gt_box_tensor = res[2] if len(res) > 2 else dataset.post_processor.generate_gt_bbx(batch_data)
    else:
