...
def load_saved_model(saved_path, model):
    ...
    if file_list:
        ...
        if hasattr(model, "load_teacher_from_stage1_state_dict") and callable(
            getattr(model, "load_teacher_from_stage1_state_dict")
        ):
            try:
                model.load_teacher_from_stage1_state_dict(loaded_state_dict)
            except Exception as e:
                print(f"[WARN] load_teacher_from_stage1_state_dict failed: {e}")
...
def setup_train(hypes):
    ...
    full_path = os.path.join("/mnt/sdb/public/data/yk/result/hetero-task/",folder_name) # change 
...
def setup_lr_schedular(hypes, optimizer, init_epoch=None):
    ...
    if lr_schedule_config['core_method'] == 'step':
        ...
        milestones = lr_schedule_config.get('step_size', None)
        if milestones is None:
            milestones = lr_schedule_config.get('milestones', None)
        if milestones is None:
            raise KeyError(
                "Missing `lr_scheduler.step_size` for multistep scheduler "
                "(accepted alias: `lr_scheduler.milestones`)."
            )
        gamma = lr_schedule_config['gamma']
