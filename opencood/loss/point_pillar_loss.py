...
class PointPillarLoss(nn.Module):
    def __init__(self, args):
        ...
        aux_cfg = args.get("aux", {})
        aux_cfg = aux_cfg if isinstance(aux_cfg, dict) else {}
        self.aux_distill_weight = float(aux_cfg.get("distill_weight", 0.0))
        self.aux_gate_weight = float(aux_cfg.get("gate_weight", 0.0))
        self.aux_gate_mean_weight = float(aux_cfg.get("gate_mean_weight", 0.0))
        self.aux_depth_weight = float(aux_cfg.get("depth_weight", 0.0))
    ...
    def forward(self, output_dict, target_dict, suffix=""):
        ...
        # Auxiliary losses (e.g., open-join protocol distillation / reliability gating).
        if self.aux_distill_weight > 0 and "distill_loss" in output_dict:
            distill_loss = output_dict["distill_loss"]
            total_loss += distill_loss * self.aux_distill_weight
            self.loss_dict.update({"distill_loss": float(distill_loss.detach().item())})

        if self.aux_gate_weight > 0 and "gate_loss" in output_dict:
            gate_loss = output_dict["gate_loss"]
            total_loss += gate_loss * self.aux_gate_weight
            self.loss_dict.update({"gate_loss": float(gate_loss.detach().item())})

        if self.aux_gate_mean_weight > 0 and "gate_mean_loss" in output_dict:
            gate_mean_loss = output_dict["gate_mean_loss"]
            total_loss += gate_mean_loss * self.aux_gate_mean_weight
            self.loss_dict.update({"gate_mean_loss": float(gate_mean_loss.detach().item())})

        if self.aux_depth_weight > 0 and "depth_loss" in output_dict:
            depth_loss = output_dict["depth_loss"]
            total_loss += depth_loss * self.aux_depth_weight
            self.loss_dict.update({"depth_loss": float(depth_loss.detach().item())})
